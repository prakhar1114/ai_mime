...
class BaseAgentChatService:
    """Base class for agent chat services, implementing core session and chat loop management."""
    ...
    def target_dir(self) -> Path:
        """The primary directory this chat service operates on (e.g., workspace_dir or workflow_dir)."""
        raise NotImplementedError

    @property
    def agent_dir(self) -> Path:
        """The directory where agent data is stored."""
        raise NotImplementedError

    @property
    def active_session_filename(self) -> str:
        """The filename used to store the active session state."""
    ...
    def _bash_approval_supported(self) -> bool:
        ...
        return bool(getattr(capabilities, "permissions", False))
    ...
    def _get_default_mode(self, session_id: str | None, existing: dict[str, Any]) -> str:
        """Return the default mode string for a session."""
        raise NotImplementedError
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        ...
        for sid, meta in index.items():
            ...
            out_by_id[sid] = {
                ...
                "mode": meta.get("mode") or self._get_default_mode(sid, meta),
        ...
        try:
            current_provider_sessions = self.session_lister(self.target_dir)
        ...
        for item in current_provider_sessions:
            ...
            merged = {
                **existing,
                ...
                "mode": existing.get("mode") or self._get_default_mode(sid, existing),
            ...
            out_by_id[sid] = {
                ...
                "mode": merged.get("mode") or self._get_default_mode(sid, merged),
    ...
    def load_messages(self, session_id: str) -> list[dict[str, Any]]:
        ...
        if runtime_id and runtime_id != self.runtime_id:
            return get_agent_runtime(runtime_id).load_messages(session_id, self.target_dir)
        return self.message_loader(session_id, self.target_dir)
    ...
    def _check_pre_stream(self) -> None:
        """Hook to run any checks before streaming starts."""
        pass

    def _post_stream_hook(self) -> dict[str, Any] | None:
        """Hook to run after the stream finishes (e.g. consuming signals)."""
        return None

    def _get_stream_temp_dir_prefix(self) -> str:
        return "ai-mime-agent-"
    ...
    async def chat_stream(
        ...
        *,
        ...
        _log(
            f"chat_stream start runtime={adapter.id} target={self.target_dir} "
        ...
        auto_allow = self._get_auto_allow_tools()
        if not self.bash_requires_approval and "Bash" not in auto_allow:
            auto_allow.append("Bash")

        async def _wrapped_pump_stream() -> None:
            try:
                with tempfile.TemporaryDirectory(prefix=self._get_stream_temp_dir_prefix()) as td:
        ...
        try:
            while True:
                ...
                if event_type == "session_started":
                    ...
                    final_status = "failed"
                    final_error = str(event.get("message") or "Runtime error.")
                ...
                # We yield the event if it's not the internal sentinel
                yield event
        finally:
        ...
        post_event = self._post_stream_hook()
    ...
    def _authorize_tool(
        ...
        # Shared permission resolving logic. Subclasses can override to add specific tools.
        if tool_name == "Skill":
        ...
        return {
            "behavior": "deny",
            "message": f"{tool_name} is not enabled.",
            "interrupt": False,
        }
    ...
    def _record_session(
        ...
        *,
        ...
        mode = self._get_default_mode(session_id, existing)
    ...
    def _read_active(self) -> dict[str, Any]:
        path = self.agent_dir / self.active_session_filename
        if not path.exists():
            return {}
        return _read_json(path)

    def _write_active(self, active: dict[str, Any]) -> None:
        _write_json(self.agent_dir / self.active_session_filename, active)
