...
2. Match `step.executor` to the execution shape:
   ...
   - `ui_agent` → first read the sibling UI-agent guide at `../ui_agent/00_ui_agent.md` under the shared instructions root, then use the computer-use tools directly to perform the step live and discover the task-specific recipe.
...
3. Execute against the live environment using `agent/confirmed_inputs.json`. Verify success (screenshots / page_info / shell output) before declaring the step done.
   - If a step needs user-specific secrets (API keys, tokens, account emails/domains — e.g. Jira), read `credentials.md` and set up the credential manifest + build-time values before running it.
4. Append durable findings to `agent/learned_notes.md` — selectors, URLs, payload shapes, traps. Make this a clear technical map of how to reproduce the step, not a diary. For any `ui_agent` steps, record a UI Agent Recipe using `../ui_agent/00_ui_agent.md`: what worked, what failed and why, focus/activation gotchas, stable visual landmarks, ambiguous selection points, skip conditions, recovery steps, known "do not do this" behaviors, and exact final verification. This recipe will be distilled into the task prompt passed to the UI agent during synthesis in Phase C.
