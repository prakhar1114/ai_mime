...
- `script`: pure deterministic Python (file IO, HTTP, parsing, library calls, shelling out to shell/CLI commands, or executing AppleScript via `osascript` subprocess calls). No UI. May call `ask_llm` for stochastic JSON-schema decisions. **This is the absolute preferred path (Priority 1: Bash/CLI/AppleScript/API/Direct Files).**
- `browser_harness`: composable Chrome CDP script via the `browser` skill / `"$AI_MIME_BROWSER_HARNESS_BIN" -c '…'`. May also call `ask_llm` for in-page judgment. **(Priority 2: Web Automation).**
...
`ask_llm` is the stochasticity escape hatch for *both* `script` and `browser_harness` steps — do not push a step to `ui_agent` just because it has one fuzzy decision. In pure Python script code use `from llm_resolver import ask_llm`; inside "browser-harness -c", "ask_llm" is preimported. Call it only as `ask_llm(prompt, schema, images=None)`. Give it an explicit JSON schema and branch deterministically on its output.
